...
3. [Thread Types](#thread-types)
4. [Thread Methods](#thread-methods)
5. [Synchronization](#synchronization)
6. [Locks](#locks)
7. [Synchronization Utilities](#synchronization-utilities)
8. [Deadlocks](#deadlocks)
9. [Executor Framework](#executor-framework)
10. [Asynchronous Programming](#asynchronous-programming)
11. [Volatile vs Atomic](#volatile-vs-atomic)
12. [Thread Communication](#thread-communication)
...
1. Only one thread can execute synchronized code at a time
2. Other threads are blocked and enter a wait queue
3. When lock is released, one thread from queue is chosen
4. Ensures mutual exclusion and prevents race conditions
...
### Deadlock Scenario:
```
Thread1: Locks Pen → Waits for Paper
           ↓
Thread2: Locks Paper → Waits for Pen
           ↑
        DEADLOCK!
```
...
#### 4. Use Specific Lock Objects
Instead of locking `this`, use specific objects.
...
1. **Executor** - Core interface for executing tasks
2. **ExecutorService** - Extended interface with lifecycle management
3. **ThreadPoolExecutor** - Implementation managing pool of threads
4. **ScheduledExecutorService** - For scheduling tasks
5. **Executors** - Utility class with factory methods
6. **Future** - Represents async computation result
7. **Callable** - Task that returns a value
...
public class ExecutorFramework {
    public static void main(String[] args) {
        ...
        System.out.println("Time without Framework: " + (System.currentTimeMillis() - startTime) + " ms");
        ...
        System.out.println("Time with Framework: " + (System.currentTimeMillis() - startTime) + " ms");
    }
...
### Thread Creation
- **Thread class** - Direct inheritance
- **Runnable interface** - Preferred approach
...
3. **Synchronization utilities** - CountDownLatch, CyclicBarrier
...
1. **Explain the difference between synchronized and Lock**
   - Synchronized: Built-in, automatic, simpler
   - Lock: More control, timeout support, fairness

2. **Why use Executor Framework?**
   - Thread pooling reduces overhead
   - Better resource management
...
5. **CountDownLatch vs CyclicBarrier?**
   - CountDownLatch: One-way, not reusable
   - CyclicBarrier: Reusable, bidirectional
